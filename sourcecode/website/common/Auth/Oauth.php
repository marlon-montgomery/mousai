...
class Oauth
{
    ...
    public function requestExtraCredentials($credentials, $service, $profile)
    {
        ...
        return $this->getPopupResponse(
            'REQUEST_EXTRA_CREDENTIALS',
            $credentials,
        );
    }
    ...
    public function validateExtraCredentials($input)
    {
        ...
        if (
            in_array('password', $credentials) &&
            !$this->callbackPasswordIsValid($input)
        ) {
    ...
    public function callbackPasswordIsValid($credentials)
    {
        try {
            if (!isset($credentials['email'])) {
                $credentials['email'] = $this->getPersistedData(
                    'profile',
                )->email;
            }
