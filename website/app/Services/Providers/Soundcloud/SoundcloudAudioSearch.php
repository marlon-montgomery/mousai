...
class SoundcloudAudioSearch {
     ...
     * @return array
     */
    public function search($trackId, $artistName, $trackName, $limit = 10, $type = 'video')
    {
    ...
    private function getBestMatches($results, $artistName, $trackName, $filterOutUnpopular = true) {
        ...
        return array_filter($results, function($track) use($artistName, $trackName, $filterOutUnpopular, $badWords) {
            ...
            $seemsLikeOriginal = ! Str::contains($title, $badWords) && ! Str::contains(strtolower($track['tag_list']), $badWords);
            $seemsLikeMatch    = Str::contains($title, strtolower($artistName)) && Str::contains($title, strtolower($trackName));
            $popularEnough     = $filterOutUnpopular ? ($track['playback_count'] > 1000 && $track['likes_count'] > 100) : true;
            $isNotDemo         = $track['duration'] !== 90757; //1:30
            $isNotLive         = Str::contains(strtolower($trackName),'live') ? true : ! Str::contains($title, 'live') && $track['track_type'] !== 'live';
